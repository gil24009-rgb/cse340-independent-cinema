...
| Server-side rendering | Home, film, screening, visit, contact, Member booking history, account, auth page EJS rendering | Express controllers and EJS | Public home, film archive, screening schedule, detail pages, visit contact form, Member booking list, auth and account pages | Automated route tests and browser inspection | In Progress |
...
| Multi-stage workflow | Member booking creation, Member booking history, and Member-owned cancellation are implemented; Staff status transitions remain next | bookings and status history | Screening detail booking action, Member booking detail, Member booking history, and cancellation state | Route, PostgreSQL transaction, browser, and conflict tests | In Progress |
| Status history | Initial booking creation and Member cancellation history are recorded in the same transaction; detail timeline remains next | booking_status_history | Booking detail and later timeline | PostgreSQL history query test | In Progress |

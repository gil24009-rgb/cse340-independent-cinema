...
| Server-side rendering | Home, film, screening, visit, contact, Member booking history and timeline, Staff booking operations, account, auth page EJS rendering | Express controllers and EJS | Public home, film archive, screening schedule, detail pages, visit contact form, Member booking list and detail timeline, Staff booking dashboard, auth and account pages | Automated route tests and browser inspection | In Progress |
...
| Multi-stage workflow | Member booking creation, Member booking history, Member-owned cancellation, Member-facing status timeline, and first Staff status transitions are implemented | bookings and status history | Screening detail booking action, Member booking detail, Member booking history, cancellation state, status timeline, and Staff booking controls | Route, PostgreSQL transaction, browser, and conflict tests | In Progress |
...
| Admin dashboard | Operational management | Aggregated queries and CRUD | Staff booking operations and later admin dashboard | Role test | In Progress |
